...
def build_vocab_files(df, categorical_column_list, default_value='00', base_path=''):
    vocab_files_list = []
    for c in categorical_column_list:
        v_file = write_vocabulary_file(df[c].unique(), c, default_value, os.path.join(base_path, 'diabetes_vocab/'))
...
def check_zero_or_null_values(df):
    null_df = pd.DataFrame({
        'total_null': df.isnull().sum(),
        'total_zero': df.isin([0]).sum()
    } )
    return null_df
